...
function generateToken(user) {
  const token = jwt.sign({ email: user.email,role: user.role,verified:user.verified }, "key", {
    expiresIn: "24h",
  });
  return token;
}
...
router.post("/signin", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: "Email est invalid" });
    }

    const passwordMatch = await bcrypt.compare(password, user.password);

    if (!passwordMatch) {
      return res.status(401).json({ message: "mot de passe est invalid" });
    }
